# C4thedral-Dokumentation

[English](../en/README.md) | [Deutsch](README.md)

Dieses Verzeichnis enthält die öffentliche deutsche Dokumentation für
C4thedral und C4ML.

## Hier anfangen

- [Benutzerhandbuch](user-guide.md) — Zweck von C4ML, ausführbare Syntax,
  Diagramme, Layout, Routing, Diagnosen und Beispiele.
- [Vibe Coding mit fachlicher Verantwortung](ki-gestuetzte-entwicklung.md) — wie
  KI das Projekt beschleunigte, welche Vorteile daraus entstanden und weshalb
  fachkundige Verantwortung zwingend bleibt.
- [Projektstatus](project-status.md) — Bedeutung der aktuellen Beta, vorhandene
  Funktionen und vorläufige Grenzen.
- [Projekte](projects.md) — Ein-Datei- und Mehrdatei-Projekte sowie typisierte
  Projektressourcen.
- [Aus dem Quellcode bauen](build-from-source.md) — C4thedral klonen, prüfen,
  starten und paketieren.

## Installation und Veröffentlichung

- [Linux-Installation](install-linux.md)
- [Windows-Installation](install-windows.md)
- [Native Plattformmatrix](platforms.md)
- [Versionshinweise für 0.1.0-beta.3](releases/0.1.0-beta.3.md)

## Am Projekt mitarbeiten

- [Mitwirken](../../CONTRIBUTING.de.md)
- [Sicherheitsrichtlinie](../../SECURITY.de.md)
- [Technische Projektdokumentation (Englisch)](../engineering/README.md)
