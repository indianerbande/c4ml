...
describe("semantic graphical authoring", () => {
  ...
  it("uses familiar questions while retaining stable C4-owned source identities", () => {
    ...
    expect(template).toContain('(click)="chooseServiceAsSoftwareSystem()"');
    expect(template).toContain('(click)="chooseServiceAsContainer()"');
    expect(template).toContain('(change)="selectServiceOwner($event)"');
    expect(template).toContain('(change)="selectServiceDiagram($event)"');
    expect(template).toContain('value="__new__"');
    expect(component).toContain('kind: "create-container-with-view"');
    ...
    expect(messages).toContain("Create as a Software System");
    expect(messages).toContain("Als Softwaresystem anlegen");
    expect(messages).toContain("Als Container in einem Softwaresystem anlegen");
    expect(messages).toContain("Create a new Container diagram");
    expect(messages).toContain("Neues Container-Diagramm erstellen");
